...
router.get('/current', requireAuth, async (req, res) => {
  try {
    ...
    const spots = await Spot.findAll({
      ...
      attributes: {
        include: [
          // Sequelize function to calculate average rating from Review model
          [
            Sequelize.fn('AVG', Sequelize.col('Reviews.stars')),
            'avgRating'
          ]
    ...
    // Return an empty array if the user is logged in but has no spots
    if (spots.length === 0) {
...
router.post('/:spotId/images', requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    // If the spot doesn't exist, return 404
    if (!spot) {
      return res.status(404).json({
        message: "Spot couldn't be found"
      });
    ...
    // If the spot exists and the user is authorized, validate input
    next();
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Something went wrong' });
